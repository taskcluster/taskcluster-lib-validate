suite('Valid Schema Tests', () => {
  ...
  test('load json', () => {
    let error = validate(
      {value: 42},
      'http://localhost:1203/schemas/whatever/v1/test-schema.json#');
  ...
  test('load yml', () => {
    let error = validate(
      {value: 42},
      'http://localhost:1203/schemas/whatever/v1/yml-test-schema#');
    assert.equal(error, null);
  });
  ...
  test('load yaml', () => {
    let error = validate(
      {value: 42},
      'http://localhost:1203/schemas/whatever/v1/yaml-test-schema#');
    assert.equal(error, null);
  });
  ...
  test('$ref', () => {
    let error = validate({
      reference: {value: 42},
      tid: new Date().toJSON(),
    }, 'http://localhost:1203/schemas/whatever/v1/ref-test-schema#');
    assert.equal(error, null);
  });
  ...
  test('default values are inserted', () => {
    ...
    let error = validate(
      json,
      'http://localhost:1203/schemas/whatever/v1/default-schema');
    assert.equal(error, null);
  ...
  test('default values with array and objects', () => {
    ...
    let error = validate(
      json,
      'http://localhost:1203/schemas/whatever/v1/default-array-obj-schema');
    assert.equal(error, null);
  ...
  test('rejects poorly formed object', () => {
    let error = validate(
      {value: 43},
      'http://localhost:1203/schemas/whatever/v1/test-schema#');
    debug(error);
  ...
  test('messages for large schema are nice', () => {
    let error = validate(
      {},
      'http://localhost:1203/schemas/whatever/v1/big-schema#');
    debug(error);
    assert.notEqual(error, null);
  });
  ...
  test('automatic id', () => {
    let error = validate(
      {value: 42},
      'http://localhost:1203/schemas/whatever/v1/auto-named-schema#');
    assert.equal(error, null);
  });
